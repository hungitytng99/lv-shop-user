...
export default function CardHistoryOrder(props) {
    const { code = "", orderItems = [], createdAt = "", customerName = "", customerAddress = "", customerPhone = "", shipFee = "", finalPrice = "" } = props.data;
    return (
        <div className="card_his_order">
            ...
            <Row style={{ borderBottom: "1px solid #f6470e" }}>
                <Col xs={12} md={6}>
                    Người nhận: {customerName} (dt: {customerPhone})
                </Col>
                        ...
                        <Row key={uuidv4()} className="mb-3">
                            <Col md={6} xs={12}>
                                <img
                                    ...
                                    alt=""
                                />
                                <span style={{ fontSize: "14px" }}>{item.variant?.product?.title}</span> <br />
                                <span style={{ fontSize: "14px" }}>{`(${item.variant?.publicTitle}) x${item.quantity}`}</span>
                            </Col>
                            <Col md={3} xs={6}>
